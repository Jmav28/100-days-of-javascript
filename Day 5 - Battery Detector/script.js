...
function initBattery() {
    ...
    navigator.getBattery().then((batt) => {
        updateBattery = () => {
            ...
            console.log(`${parseInt(batt.level * 100)} %`);
            if(level == 100) {
                batteryStatus.innerHTML = `Battery Full < class="ri-battery-2-fill green-color"></i>`;
                batteryLiquid.style.height = "103%";
            } else if(level <= 20 &! batt.charging) {
                batteryStatus.innerHTML = `Low Charge < class="ri-plug-line animated-red animated-red"></i>`;
            } else if(batt.charging) {
                batteryStatus.innerHTML = `Charging... < class="ri-flashlight-line animated-green"></i>`;
            } else {
